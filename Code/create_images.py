...
"""
Takes Fasta or Simulation files and generates images for CNN analysis
"""
...
# def analysis_focus():
# 	"""
# 	Elementry function to get the direction of thw workflow: NOTE: simplistic idea of the workflow
	
# 	Returns: 
# 		workflow (string) -- whether simulated data or real data is being used

# 	"""
# 	work_direction = raw_input('Real or Simulations?')
# 	work_direction = work_direction.strip()
# 	work_direction = work_direction.lower()
# 	return work_direction;

def get_gene():
	"""
	Function to get the name of the gene being investigated: NOTE: simplistic idea of the workflow
	
	Returns: 
		gene_name (string) -- name of the gene under investigation

	"""
	order = 'Type the name of the gene under investigation:'
...
# 	"""
# 	order = 'Type the path to the %s file: ' %(filetype)
...
def create_results_folder(results_folder_path, record_file_path,gene = 'na'):
	"""
	Checks if simulation or fasta specific results folder exists and if not creates one
	Checks if gene specific folder exists and if not creates one
	Checks if simulation_file specific folder exists and if not creates one
	
	Keyword Arguments: 
		results_folder_path (string) -- path to the results folder (Does not yet assume specific directory structure)
		record_file_path (string) -- name of file being processed
		gene (string) -- name of the gene under investigation ( has default value if not called)
...
def user_choice(subject):
	"""
	Function to allow user to specify if operation is to be applied
	
	Keyword Arguments: 

		subject (string) -- question to be asked to the user
	
	Returns: 
		user_apply (boolean) -- True or False 

	"""
	while True:
		order = '%s: (y/n)' %(subject)
		input_apply= raw_input(order)
		input_apply  = input_apply.strip()
		if input_apply is not 'y' and input_apply is not 'n':
...
def process_fasta(ref,records,alleles,p_order,populations,threshold,delete_d,delete_mono,delete_bi,delete_tri,delete_quadri):
	"""
...
loop_1=True      
while loop_1:
	option_1, option_2 = options_menu("Raw Data Type", ['Simulation Files','Fasta Files'])
	data_type = input("Enter your choice [1-2]: ")
	if data_type==1:
		print width_screen * "-"     
...
if data_type == 2:
	...
	populations = []
	gene = get_gene()
	reference_file_path = getfile('reference')
	record_file_path = getfile('records')
	results_folder_path = getfile('results_folder')
	...
	p_threshold  = get_threshold('p_threshold',0 ,1)
	threshold  = get_threshold('threshold',0 ,1)   
	apply_threshold = user_choice('Do you wish to apply the theshold value?')
	#Process the Fasta files
	populations,matrix,tot_n_gen,ref_genome,freq,Mm,n_positions,n,n_alleles = process_fasta(ref,records,alleles,p_order,populations,threshold,False,True,False,False,False)
	#Generate the images
...
if data_type == 1:
	...
	if os.path.isdir(data_input):
		...
		if file_error:
			...
			with open(simulations_results_outout + '/errors/file_error.csv', 'wb') as output:
